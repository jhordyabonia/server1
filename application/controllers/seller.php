...
class Seller extends CI_Controller {
	...
	public function i($v=1)
	{
		...
		{
			if(false!=$this->ventor->get(array('correo'=>$new_ventor['correo'])))
			{	$err="El correo ya está registrado "; }
			else if(is_object($this->ventor->get(array('cc'=>$new_ventor['cc']))))
			{	$err="El documento ya está registrado "; }
			else if(is_int($new_ventor['id']))
	...
	public function tablero_usuario()
	{		
		if($this->session->userdata('nombres')=='')
			redirect(base_url());
	...
	{		
		$d['titulo']='Términos y Condiciones';
		$this->load->view('template/head',$d);
		$this->load->view('template/terminos',$d);
		$this->load->view('template/footer',$d);
	}
